({
    CheckTabsCompletion : function(component) {
        ...
        this.CheckServicePackage(component);
    ...
    CheckpaymentsCompletion : function(component) {
        ...
        CheckServicePackage : function(component) {
        var action = component.get("c.checkServicePackage");
        action.setParams( {
            recordId : component.get("v.recordId")
        });
        action.setCallback(this, function(response) {
            var state = response.getState();
            console.log('check service PAckage ',state);
            if (state === "SUCCESS") {
                var responseValue = response.getReturnValue();
                component.set("v.servicePackageCompletion",responseValue);
